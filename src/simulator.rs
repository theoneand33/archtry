...
use crate::utils::{
    create_progress_bar, get_user_input, get_user_input_with_prompt, show_header, show_success,
    show_warning, sleep,
};
...
pub fn run_simulation(user_choices: &UserChoices, no_hints: bool) {
    ...
    if let DeviceType::Laptop = user_choices.device_type {
        simulate_wifi_setup(no_hints);
    }

    simulate_partitioning(no_hints);
    simulate_mount(no_hints);
    simulate_pacstrap(&user_choices.gpu_type, no_hints);
    simulate_final_steps(no_hints);
}

/// Simulate Wi-Fi setup for laptops
fn simulate_wifi_setup(no_hints: bool) {
    show_header("Setting up Wi-Fi for laptop...");
    show_warning("Note: You need to manually connect to Wi-Fi.");

    simulate_command("ip a", "Show network interfaces", false, no_hints, || {
        println!(
            r#"
...
"#
    ...
    simulate_command(
        "iwctl station wlan0 connect archtry_wifi",
        "Connect to Wi-Fi",
        false,
        no_hints,
        || {
...
fn simulate_partitioning(no_hints: bool) {
    show_header("Partitioning disk...");
    show_warning("Note: You need to manually partition the disk.");

    simulate_command("lsblk", "Show disk layout", false, no_hints, || {
        println!(
            r#"
...
fn simulate_mount(no_hints: bool) {
...
fn simulate_pacstrap(gpu_type: &GpuType, no_hints: bool) {
...
fn simulate_final_steps(no_hints: bool) {
    ...
    set_password(&username, in_chroot, no_hints);

    choose_environment(in_chroot, no_hints);
    ...
    simulate_command("reboot", "Reboot the system", false, no_hints, || {
        println!("\nSystem rebooting...\n");
    });

    println!(
        "\n{}",
        "Installation complete! You learned 40-60% of how to install Arch Linux manually!"
            .bright_green()
    );
    println!(
        "{}",
        "This is a fake simulation. For a complete guide, visit:".bright_blue()
    );
    println!(
        "{}",
        "https://wiki.archlinux.org/title/Installation_guide".bright_blue()
    );
}

/// Set a password for a user
fn set_password(username: &str, in_chroot: bool, no_hints: bool) {
    loop {
        ...
        println!(
            "{}",
            format!("Run '{}' to set the password for '{}':", command, username).bright_blue()
        );
        println!(); // Add spacing
        ...
        if input.contains(command) {
            ...
            if password == confirm_password {
                println!(
                    "{}",
                    format!("Password set for '{}'.", username).bright_green()
                );
                break;
            } else {
            ...
            if !no_hints {
                println!(
                    "{}",
                    "Hint: Make sure you type the command exactly as shown.".bright_cyan()
                );
            }
        }
    }
...
fn choose_environment(in_chroot: bool, no_hints: bool) {
    ...
    simulate_command(
        &format!("pacman -S {}", package),
        &format!("Install {}", options[selection]),
        in_chroot,
        no_hints,
        || {
            let total_steps = 7;
...
fn simulate_command<F>(command: &str, description: &str, in_chroot: bool, no_hints: bool, action: F)
where
    F: FnOnce(),
{
    ...
    if input.trim() == command {
        ...
        simulate_command(command, description, in_chroot, no_hints, action); // Retry
    }
}
